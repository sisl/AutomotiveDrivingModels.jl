...
- construct split over traces
- run CV split over drives to get the best hyperparams for each model
- for each trace:
...
for dset_filepath_modifier in (
    ...
    split_traces = get_fold_assignment_across_traces(dset)
    ...
    #  - update trace metrics on given trace
    #  - update mean frame likelihood

    nframes = nrow(dset.dataframe)
    ntraces = split_traces.nfolds
    ...
    tic()
    for fold in 1 : ntraces
        println("Fold: ", fold, "  /  ", ntraces)

        print("\ttraining models  "); tic()
        training_frames = dset.dataframe[split_traces.frame_assignment.!=fold, :]
        training_frames_nona = dset.dataframe_nona[split_traces.frame_assignment.!=fold, :]
        ...
        for (k,behavior) in enumerate(models)
            ...
            for i in 1 : ntraces
                if split_traces.pdsetseg_assignment[i] == fold # in test
                    # simulate
    ...
    let
        ...
        for k in 1:nmodels
            ...
            for j in 1 : split_traces.nfolds
                for i in 1 : nframes
                    if split_traces.frame_assignment[i] == j
