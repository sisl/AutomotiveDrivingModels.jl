...
for (csvfileset_index, csvfileset) in enumerate(CSVFILESETS)
    ...
    for runlog in runlogs
        ...
        for frame in 1 : nframes(runlog)
            ...
            tot_freeflow += is_behavior_fag_set(runlog, colset, frame, ContextClass.FREEFLOW)
            tot_following += is_behavior_fag_set(runlog, colset, frame, ContextClass.FOLLOWING)
            tot_lanechange += is_behavior_fag_set(runlog, colset, frame, ContextClass.LANECHANGE)
        end
    end
end
toc()
