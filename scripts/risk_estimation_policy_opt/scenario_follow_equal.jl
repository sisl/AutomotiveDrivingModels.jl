scenario = let
    ...
    trajs[2] = TrajDef(sn, VecE2(x₀+delta_x,-4.5), speed_65mph)
    push!(trajs[2], TrajDefLinkTargetSpeed(history, lanetagR, 0.0, speed_65mph))
    push!(trajs[2], TrajDefLinkTargetSpeed(horizon, lanetagR, 0.0, speed_65mph))

    Scenario("follow_equal", sn, history, DEFAULT_SEC_PER_FRAME, trajs)
end