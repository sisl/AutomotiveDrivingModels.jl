let
    ...
    s = RoadwayState(VecSE2(0.0,0.0,0.0), Frenet(NULL_ROADINDEX, 0.0, 0.0, 0.0), 10.0)
    @test isapprox(get_vel_s(s), 10.0)
    @test isapprox(get_vel_t(s),  0.0)
    show(IOBuffer(), s.posF)
    show(IOBuffer(), s)

    s = RoadwayState(VecSE2(0.0,0.0,0.0), Frenet(NULL_ROADINDEX, 0.0, 0.0, 0.1), 10.0)
    ...
    @test RoadwayState(VecSE2(0.1,0.2,0.3), 1.0) == RoadwayState(VecSE2(0.1,0.2,0.3), NULL_FRENET, 1.0)
end
