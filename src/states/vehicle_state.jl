...
"""
get_front(veh::Entity{VehicleState, D, I}) where {D<:AbstractAgentDefinition, I} = veh.state.posG + polar(veh.def.length/2, veh.state.posG.θ)

"""
    get_rear(veh::Entity{VehicleState, VehicleDef, I})
returns the position of the rear of the vehicle
"""
get_rear(veh::Entity{VehicleState, D, I}) where {D<:AbstractAgentDefinition, I} = veh.state.posG - polar(veh.def.length/2, veh.state.posG.θ)
