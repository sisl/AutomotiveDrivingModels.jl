...
"""
@with_kw mutable struct  <: DriverModel{PedestrianLatLonAccel}
    timestep::Float64
...
function update_crossing(ped::Vehicle,
    ...
    if Δs < 0.1
        stop_ped(ped, model, model.crosswalk)
        model.phase = LEAVING
    end
end
...
function stop_ped(ped::Vehicle, model::SidewalkPedestrianModel, lane_des::Lane)
    model.a = PedestrianLatLonAccel(0.0, calc_a(ped.state.v, 0.0, model.timestep), lane_des)
end

function calc_a(Δs::Float64, v::Float64, Δt::Float64)
    return 2(Δs - vΔt)/Δt^2
end
