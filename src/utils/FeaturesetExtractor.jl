...
function extract_csvfile_sets(
    ...
    carid = CARID_EGO # TODO(tim): URGENT - to use non-ego we must fix feature extraction
        # println(carid)
...
function _get_streetmap(streetmapbasename::String, streetnet_cache::Dict{String, StreetNetwork})
...
function extract(
    ...
    sn = _get_streetmap(csvfileset.streetmapbasename, streetnet_cache)
