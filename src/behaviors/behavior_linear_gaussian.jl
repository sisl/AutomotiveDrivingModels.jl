...
function train_special(::Type{VehicleBehaviorLinearGaussian}, trainingframes::DataFrame;
    ridge_regression_constant::Float64=0.5,
    args::Dict=Dict{Symbol,Any}()
    )

    for (k,v) in args
        if k == :ridge_regression_constant
            ridge_regression_constant = v
        else
            warn("Train VehicleBehaviorLinearGaussian: ignoring $k")
        end
    ...
    indicators = filter(s->s!=:f_des_angle_250ms && s!=:f_accel_250ms, names(trainingframes))
    for (predictor_index, sym) in enumerate(indicators)
    ...
    VehicleBehaviorLinearGaussian(best_predictor_index, best_A, best_μ∞, N)
end