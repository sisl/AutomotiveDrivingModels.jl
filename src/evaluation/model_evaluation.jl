...
function optimize_hyperparams_cyclic_coordinate_ascent(
    ...
    training_params::AbstractVehicleBehaviorTrainParams, # already set with default values (overwritten)
    cross_validation_split::FoldAssignment,
    behavior_param_set::Vector{BehaviorParameter},
    )

    if isempty(behavior_param_set) # nothing varies
        return training_params
    end

    n_varying_params = length(behavior_param_set)
    param_indeces = Array(Int, n_varying_params)
    for i in 1 : n_varying_params
        param_indeces[i] = behavior_param_set[i].index_of_default
    end
    ...
    while !finished
        ...
        for (param_index, behavior_param) in enumerate(behavior_param_set)

            sym = behavior_param.sym
            best_param_val = getfield(training_params, sym) # current one
            ...
            for (i, v) in enumerate(behavior_param.range)
                ...
                if !in(param_hash, param_hash_set)
                    ...
                    setfield!(training_params, sym, v)

                    logl = 0.0
                    for fold in 1 : cross_validation_split.nfolds
                        model = train(dset, preallocated_data, training_params, fold, cross_validation_split, false)
            ...
            setfield!(training_params, sym, best_param_val)
        end
