	...
	@eval begin
		function _get(::$feature_name, pdset::PrimaryDataset, ::StreetNetwork, carind::Int, validfind::Int)
			...
			if carind == CARIND_EGO
				curr = _get(TURNRATE, pdset, CARIND_EGO, validfind_past1)
				past = _get(TURNRATE, pdset, CARIND_EGO, validfind_past2)
			elseif idinframe(pdset, carid, validfind_past1) && idinframe(pdset, carid, validfind_past2)
				...
				curr = _get(TURNRATE, pdset, ind1, validfind_past1)
				past = _get(TURNRATE, pdset, ind2, validfind_past2)
			else
				return NA_ALIAS
			end

			(curr - past)/(SEC_PER_FRAME*N_FRAMES_PER_SIM_FRAME)
		end
