	...
	@eval begin
		function _get(::$feature_name, pdset::PrimaryDataset, ::StreetNetwork, carind::Int, validfind::Int)
			...
			if carind == CARIND_EGO
				curr = get(SPEED, pdset, CARIND_EGO, validfind_past1)
				past = get(SPEED, pdset, CARIND_EGO, validfind_past2)
			elseif idinframe(pdset, carid, validfind_past1) && idinframe(pdset, carid, validfind_past2)
				...
				curr = get(SPEED, pdset, ind1, validfind_past1)
				past = get(SPEED, pdset, ind2, validfind_past2)
			else
				return NA_ALIAS
			end
